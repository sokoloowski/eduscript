...
if ($('link[rel="stylesheet"]:last-child').attr("href").match(/light/gi)) {
    var iconColor = "black";
} else if (
    $('link[rel="stylesheet"]:last-child').attr("href").match(/dark/gi)
) {
    var iconColor = "white";
}
...
setInterval(() => {
    navigator.getBattery().then(function (battery) {
        ...
        function updateChargeLevel() {
            ...
            if (battery.level >= 0 && status == "charging")
                $("#battery")
                    .html('<i class="material-icons">battery_charging_full</i>')
                    .css("color", "white");
            if (battery.level >= 0.9 && status == "charging")
                $("#battery").hide();

            $("#battery").attr("title", `${Math.round(battery.level * 100)}%`);
            if (time == "00:00:00")
                $(".control.battery>div").html(
                    `${Math.round(battery.level * 100)}%, ${status}`
                );
            else
                $(".control.battery>div").html(
                    `${Math.round(
...
for (v of appList) {
    $(`<script src="apps/${v}/app.js"></script>`).insertBefore(
        "script:last-child"
    );
}
...
bar = (icon, title, style) => {
    ...
    } else if (style.match(/[0-9a-f]{6}/gi)) {
...
newWindow = (name, icon, title, content, style) => {
    if (style == "alert" || style == "transparent") {
        $("#workspace").append(
            `<div class="window ${style}" name="${name}">${bar(
                icon,
                title,
                style
        ...
        $("#workspace").append(
            `<div class="window" name="${name}">${bar(
                icon,
                title,
                style
            )}<div class="content">${content}</div></div>`
...
$("#workspace").on("click", ".state.minimize", function () {
    ...
    $(
        `.enabled[name="${$(this).closest(".window").attr("name")}"]`
    ).toggleClass("active");
});
...
$("#workspace").on("click", ".state.maximize", function () {
    $(this)
        .closest(".window")
        .toggleClass("maximize ui-draggable ui-draggable-handle");
});
...
checkWindows = (n, c) => {
    if (n != undefined) {
        ...
        $("#navbar>.activators").append(
            `<div class="enabled active" title="${eval(n).title}" name="${
                eval(n).name
            }" style="background-image:url(${eval(n).icon.replace(
                "-black",
                "-white"
            )})"></div>`
        );
    } else {
        $("#navbar>.activators").html(
            '<div class="menu"><i class="material-icons">apps</i></div>'
        );
    }
};
